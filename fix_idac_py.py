...
# 3. Add Accordion Wrapper for Contacts
code = re.sub(
...
# 4. Transform end of Contacts / start of Shift Times
code = re.sub(
    r'\s*<\/div>\s*<\/div>\s*\) : activeTab === \'SHIFT_TIMES\' \? \(\s*\/\* SHIFT TIME MANAGEMENT TAB \*\/\s*<div className="space-y-4">',
    '''
              </div>
            )}
          </div>
...
# 5. Transform end of Shift Times / start of Responsibilities
code = re.sub(
    r'\s*<\/div>\s*<\/div>\s*\) : activeTab === \'RESPONSIBILITIES\' \? \(\s*\/\* RESPONSIBILITIES TAB \*\/\s*<div className="space-y-5">',
    '''
              </div>
            )}
          </div>
...
# 6. Transform the end of the file
code = re.sub(
    r'\s*<\/div>\s*<\/div>\s*\)\s*:\s*null\}\s*<\/div>\s*\{\/\* Footer \*\/\}',
    '''
              </div>
            )}
          </div>
...
# Remove the Seniority order text block
code = re.sub(
    r'\{\/\* Seniority & Instructions Info \*\/\}\s*<div className="p-3 bg-emerald-50\/60.*?<\/div>\s*<\/div>\s*<\/div>',
    '',
    code,
    flags=re.DOTALL
)
...
# Import ChevronDown and BookOpen if not imported
