  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 rounded-none sm:rounded-3xl w-full max-w-[400px] border-0 sm:border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col h-full sm:h-auto">
        ...
        {/* Footer */}
        <div className="p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex flex-col space-y-3">
          <div className="flex justify-between items-center px-2 py-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
             <span className="text-sm font-bold text-slate-600 dark:text-slate-400 ml-2">Total Assigned:</span>
             <div className="bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 font-black px-3 py-1 rounded-lg">
                {totalFlightSelected}
             </div>
          </div>
          <button
...
console.log('Patched FlightDutyCalendarModal.tsx');
