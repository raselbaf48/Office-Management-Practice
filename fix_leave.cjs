...
function fixLeave() {
  ...
  code = code.replace(
    /\{currentList\.length > 0 \? \([\s\S]*?<\/div>\s*\) : \(\s*<div className="flex-1 flex items-center justify-center text-xs text-slate-400 font-medium">\s*Nobody on Leave today\s*<\/div>\s*\)\}/,
...
fixLeave();
console.log('Fixed Leave Register currently on leave box');
