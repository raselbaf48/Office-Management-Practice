...
content = content.replace(startRegex, (fullMatch) => {
  ...
  replaced = replaced.replace(/<input[\s\S]*?className="[^"]+"[\s\S]*?\/>/, `<div className={\`w-4 h-4 rounded flex items-center justify-center shrink-0 transition-colors \${isChecked ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600 border'}\`}>
