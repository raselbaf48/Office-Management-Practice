...
code = code.replace(
  /<div className="fixed inset-0 z-\[60\].*?>/g,
...
code = code.replace(
  /className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-6xl max-h-\[90vh\] shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden relative"/g,
...
code = code.replace(
  /<button[\s\S]*?onClick=\{onClose\}[\s\S]*?<X className="w-5 h-5"\s*\/>\s*<\/button>/g,
