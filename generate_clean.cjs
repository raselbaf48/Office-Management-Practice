...
const top = orig.substring(0, orig.indexOf('const sections = ['));
...
const sectionsStart = orig.indexOf('const sections = [');
const sectionsEnd = orig.indexOf('  return (', sectionsStart);
const sectionsBlock = orig.substring(sectionsStart, sectionsEnd);
...
fs.writeFileSync('src/components/SettingsModal.tsx', top + stateBlock + sectionsBlock + renderStart + '\n\n' + cloudSyncBlock + '\n\n' + usersBlock + '\n\n' + securityBlock + '\n\n' + databaseBlock + '\n\n' + historyBlock + newRenderEnd);
