...
// Remove Tab Switcher entirely
const tabSwitcherRegex = /\{\/\* Tab switcher \*\/\}\s*<div className="px-6 pt-3 border-b border-slate-200[\s\S]*?<\/div>\s*\{\/\* Toast Banner \*\/\}/;
code = code.replace(tabSwitcherRegex, '{/* Toast Banner */}');

// Restructure the content body
const bodyRegex = /\{\/\* Content Body \*\/\}\s*<div className="p-6 max-h-\[62vh\] overflow-y-auto space-y-4">\s*\{activeTab === 'CONTACTS' \? \(/;
...
const newBody = `
        ...
        <div className="p-5 sm:p-6 max-h-[62vh] overflow-y-auto space-y-3">
          ...
          <div className={\`rounded-2xl border transition-all duration-200 overflow-hidden \${
            ...
            {activeTab === 'CONTACTS' && (
              <div className="p-4 pt-0 border-t border-slate-100 dark:border-slate-800/60 animate-fadeIn">
`;

code = code.replace(bodyRegex, newBody);

// Replace the end of CONTACTS and start of SHIFT_TIMES
const shiftTimeRegex = /\) : activeTab === 'SHIFT_TIMES' \? \(\s*\{\/\* SHIFT TIME MANAGEMENT TAB \*\/\}\s*<div className="space-y-4">/;
...
code = code.replace(shiftTimeRegex, newShiftTime);

// Replace the end of SHIFT_TIMES and start of RESPONSIBILITIES
const respRegex = /\) : activeTab === 'RESPONSIBILITIES' \? \(\s*\{\/\* RESPONSIBILITIES TAB \*\/\}\s*<div className="space-y-5">/;
...
code = code.replace(respRegex, newResp);

// Replace the end of RESPONSIBILITIES
const endRegex = /<\/div>\s*\) : null\}\s*<\/div>\s*<\/div>\s*<\/div>\s*\);\s*\};\s*$/;
const newEnd = `
              </div>
            )}
          </div>
...
// Also need to import ChevronDown and BookOpen if not imported
if (!code.includes('ChevronDown')) {
  code = code.replace(/import \{([\s\S]*?)\} from 'lucide-react';/, "import { ChevronDown, BookOpen, $1 } from 'lucide-react';");
}
