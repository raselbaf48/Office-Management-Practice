...
import { FLYING_WING_UNITS, FlightName } from '../types';
    ...
    const newData = unitsData.map(d => {
      if (d.unit === selectedUnit) {
        ...
        Object.entries(formDisposalValues).forEach(([k, v]) => {
          if (v > 0) {
            if (k === 'Det/Tdy') newDetTdy = v;
            else newDisposals[k] = v;
          }
        });
        
        return {
          ...d,
          totalStr: formTotalStr,
          detTdy: newDetTdy,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs no-print overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-md my-8 text-slate-900 dark:text-white flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleAddSubmit} className="p-5 overflow-y-auto flex-1 space-y-6">
              ...
              {/* Only show below if unit is selected */}
              {selectedUnit && (
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-emerald-500" 
                    />
                  </div>
                    ...
                    <div className="space-y-3">
                      {formSavedDisposals.map((cat) => (
                        <div key={cat} className="flex items-center gap-2 group">
                          ...
                          <input
                            ...
                            onChange={(e) => setFormDisposalValues({...formDisposalValues, [cat]: parseInt(e.target.value) || 0})}
                      ...
                      <div className="pt-2 relative">
                        <button
                          ...
                          className="px-3 py-2 rounded-xl text-xs font-bold border border-dashed border-slate-300 dark:border-slate-600 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:border-slate-400 bg-slate-50 dark:bg-slate-900 transition-all cursor-pointer flex items-center space-x-1"
                        >
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 shrink-0">
              <button
                ...
                className={`w-full py-2.5 rounded-xl text-sm font-black shadow-xs transition-all ${
                  !selectedUnit
